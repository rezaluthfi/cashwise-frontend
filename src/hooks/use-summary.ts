...
export const useSummary = (
  ...
  const calculateSummary = () => {
    // Return default values if transactions is undefined
    if (!transactions) {
      return {
        totalIncome: 0,
        totalExpenses: 0,
    ...
    const cumulativeBalanceCurrent =
      calculateCumulativeBalance(currentMonthEndDate);
    const cumulativeBalancePrevious =
      calculateCumulativeBalance(prevMonthEndDate);
    const cumulativeBalanceDifference =
      cumulativeBalanceCurrent - cumulativeBalancePrevious;
...
// Tambahkan tipe untuk hasil summary
export type Summary = {
  totalIncome: number;
